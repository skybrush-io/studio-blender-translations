...
3. In "Properties" panel / "Render" / "I18n Update Translation" press the "Import PO..." button under the "Add-ons" section:
    1. Select this repository's `po` directory for the PO inputs/outputs (and ensure you are on the `main` branch of this repo).
    ...
    4. Finally, run the `etc/scripts/sort_translations.py` script and follow its instructions to try to create a more-or-less
       deterministic output of the translation strings. Unfortunately the official addon does not guarantee that thus this
       script cannot either, but it still reduces some of the noise in the output.
...
5. Make sure that the exported files are all utf-8 encoded. Run `scripts/convert-encoding.ps1` to ensure correct encoding.
6. Also run the `scripts/sort-po.py` script to sort and clean all .po and .pot files to have minimal diff.
7. Make sure your updates on the `main` branch are synched with the `weblate` branch used by https://translate.skybrush.io.
8. Finally, perform steps 3.3 and 3.4 from the previous section to have a synchronized translation database in the `skybrush-studio-blender` and `skybrush-studio-blender-translations` repositories.
